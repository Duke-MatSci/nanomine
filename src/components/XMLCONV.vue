<template>
  <div class="main">
    <div class="xmlconv">
      <h1 v-if="this.$store.state.versionNew" class="header-mm">Data Uploader</h1>
      <v-container>
        <h1 v-if="!this.$store.state.versionNew" class="header-nm">File Upload</h1>
        ...
        <p class="text-xs-left">The simplest method to curate your sample into the database is by uploading an MS Excel
          ...
          <router-link v-if="!this.$store.state.versionNew" to="/contact">the NanoMine team</router-link>
          <router-link v-else to="/mm/contact">the NanoMine team</router-link> if customization is required.
        </p>
        <br>
        ...
        <p v-if="referenceOpen" class="text-xs-left">Zhao, H., Li, X., Zhang, Y., Schadler, L. S., Chen, W., &amp; Brinson, L. C. (2016). <i><a href="https://aip.scitation.org/doi/abs/10.1063/1.4943679">Perspective: NanoMine: A material genome approach for polymer nanocomposites analysis and design</a></i>. APL Materials, 4(5), 053204.</p>
        <p v-if="referenceOpen" class="text-xs-left">Zhao, H., Wang, Y., Lin, A., Hu, B., Yan, R., McCusker, J., ... &amp; Brinson, L. C. (2018). <i><a href="https://aip.scitation.org/doi/10.1063/1.5046839">NanoMine schema: An extensible data representation for polymer nanocomposites</a></i>. APL Materials, 6(11), 111108.</p>
      </v-container>
    </div>
  </div>
</template>
