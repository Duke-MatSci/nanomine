...
if rc == 0: # send success email
  try:
    ...
    emaildata = {
      ...
      "emailvars": {
        "resultpage": webBaseUri + '/nm#/IntelligentCharacterizeResults?refuri='+jobDataUriSuffix+'/'+jobId,
        "jobinfo": {
          "resultcode":rc
        },
        "user": userId
    ...
    logging.info('exception occurred sending run_IntelligentCharacterize success email')
    logging.info('exception: ' + traceback.format_exc())
  ...
  try:
    ...
    logging.info('exception occurred sending run_IntelligentCharacterize failure email')
    logging.info('exception: ' + traceback.format_exc())
