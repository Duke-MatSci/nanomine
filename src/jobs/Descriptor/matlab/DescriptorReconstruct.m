    ...
    switch str2num(input_type)
        case 1
            ...
            if max(img(:)) > 1
                ...
                img = round(img_original/256);
            else
                imwrite(256*img,[path_to_write,'/','Input1.jpg'])
            end
            
        case 2
            unzip([path_to_read,file_name],[path_to_write,'/input']); % Umar - i think there is something missing here. The file is not even read.
        case 3
            path=[path_to_read,file_name];
            k=load(path);
            [no_need,f_name,ext]=fileparts(file_name);
            try
                img_original = getfield(k,f_name);
            catch ex
                rc = 98;
                msg = getReport(ex);
