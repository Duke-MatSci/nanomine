...
if rc == 0: # send success email
  try:
    ...
    emaildata = {
      ...
      "emailvars": {
        "resultpage": webBaseUri + '/nm#/DescriptorReconstructResults?refuri='+jobDataUriSuffix+'/'+jobId,
        "jobinfo": {
          "resultcode":rc
        },
        "user": userId
    ...
    logging.info('exception occurred sending run_DescriptorReconstruct success email')
    logging.info('exception: ' + traceback.format_exc())
  ...
  try:
    ...
    logging.info('exception occurred sending run_DescriptorReconstruct failure email')
    logging.info('exception: ' + traceback.format_exc())
